...
public class RedisClusterStore<K extends Comparable<K>, V> implements Store<K, V> {

    private final RedisClusterReactiveCommands<String, Object> commands;
    private final String storeName;

    public RedisClusterStore(StatefulRedisClusterConnection<String, Object> connection, String storeName) {
    ...
    public Mono<V> find(K id) {
        return Mono.defer(() -> commands.hget(storeName, createKey(id)).map(RedisClusterStore::cast));
    }
    ...
    @Override
    public Flux<K> keys() {
        return Flux.defer(() -> commands.hkeys(storeName)).map(RedisClusterStore::cast);
    }

    @Override
    public Flux<V> values() {
        return Flux.defer(() -> commands.hvals(storeName)).map(RedisClusterStore::cast);
    }
